...
- multiple folder arrays, example of options
        -all: do everything
...
def augment(target, out = None):
    ...
    size, threshold = augmentation_round(gaussian_path, out, count, 'threshold', augment_threshold)
    count = count + size
    
    size, canny = augmentation_round(threshold, out, count, 'canny', augment_canny)
    count = count + size
    '''
